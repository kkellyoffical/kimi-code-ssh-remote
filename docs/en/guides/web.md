...
The first connection also trusts the remote's host key automatically (`accept-new`) and every later connection verifies it; if the key ever changes, `connect` stops with a stored-vs-presented fingerprint comparison — see [Host key verification](../reference/kimi-command.md#host-key-verification) for how to review and accept the new key.
